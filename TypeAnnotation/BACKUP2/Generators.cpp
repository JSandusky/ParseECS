...
std::string PrintCalls(ReflectedType* type)
{
    ...
    for (auto method : type->methods_)
    {
        ...
        ret << "    Variant ret;\r\n";
        if (method->returnType_->typeName != "void")
            ret << "    ret = ";
        else
            ret << "    ";
        ...
        for (unsigned i = 0; i < method->argumentTypes_.size(); ++i)
        {
            ...
            ret << "args.size() > " << i << " ? args[" << i << "]." << VariantGetter(argType->type_->typeName) << " : " << argType->type_->variantDefault_;
        }
        ret << ");\r\n";
        ret << "    return ret;\r\n";
        ret << "}\r\n";
...
std::string PrintImgui(ReflectedType* type, ReflectionDatabase* database)
{
    ...
    for (auto property : type->properties_)
    {
        ...
        {
            if (property->type_ == database->GetType("int"))
            {
            ...
            else if (property->type_ == database->GetType("float"))
            {
                ss << "    ImGui::DragFloat(\"" << textName << "\", &object->" << propName << ");\r\n";
            }
            else if (property->type_ == database->GetType("std::string"))
            {
                ss << "{ static char data[4096]; memcpy(data, object->" << propName << ".c_str(), object->" << propName << ".length()); if (ImGui::InputText(\"" << textName << "\", data, 4096); }\r\n";
            }
        }
    }
