...
/// This is necessary because of templates, superficially it may look like properties could handle it
/// like how Methods use Properties for arguments, but it doesn't actually work out that well for properties.
struct TypeInstance {
    ...
    std::vector< std::pair<TypeInstance, int> > templateParameters_;
    /// Extra binding data.
    std::vector<std::string> bindingData_;

    bool IsVoid() const { return type_ == nullptr; }

    std::string GetTypeName() const;

    bool Is(ReflectedType* t) const;
    bool Is(AccessModifiers) const;
    bool IsNot(ReflectedType* t) const { return !Is(t); }
    bool IsNot(AccessModifiers a) const { return !Is(a); }
    bool Is(const std::vector<AccessModifiers>& a) const {
        for (auto aa : a)
            if (!Is(aa))
                return false;
        return true;
    }
    bool IsNot(const std::vector<AccessModifiers>& a) const { return !Is(a); }
    bool IsOneOf(const std::vector<ReflectedType*>& t) const {
        for (auto tt : t)
            if (Is(tt))
                return true;
        return false;
    }
    bool IsNotOneOf(const std::vector<ReflectedType*>& t) const { return !IsOneOf(t); }

    std::string ToString(bool withModifiers) const;
};
...
struct Property
{
    ...
    TypeInstance typeHandle_;
    /// Where do flags bits come from?
    ReflectedType* enumSource_;
    ...
    bool Is(AccessModifiers mod) const { return typeHandle_.accessModifiers_ & mod; }

    bool IsNot(AccessModifiers mod) const { return (typeHandle_.accessModifiers_ & mod) == 0; }

    bool Is(const std::vector<AccessModifiers>& mod) const {
        for (auto m : mod)
            if (!(m & typeHandle_.accessModifiers_))
                return false;
        return true;
    }
    ...
    bool IsReadOnly() const { return typeHandle_.accessModifiers_ & AM_Const || !HasBindingProperty("set"); }

    bool IsTemplate() const { return typeHandle_.templateParameters_.empty(); }
};

struct Method
{
    /// Return type.
    TypeInstance returnTypeHandle_;
    ...
    bool IsReturning() const { return !returnTypeHandle_.IsVoid(); }
};
...
struct ReflectedType
{
    ...
    std::vector<TypeInstance> baseClass_;
    ...
    bool InheritsFrom(const ReflectedType* t) const {
        ...
        {
            if (bt.type_ == t) // is this guy our target?
                return true;
            if (bt.type_->InheritsFrom(t)) // recurse
                return true;
        }
        return false;
    }
    ...
    bool GetInheritanceTreeTo(const ReflectedType* t, std::vector<ReflectedType*>& holder) const {
        ...
        for (auto bt : t->baseClass_)
        {
            if (bt.type_ == t)
            {
                holder.push_back(bt.type_);
                return true;
            }

            holder.push_back(bt.type_);
            if (bt.type_->GetInheritanceTreeTo(t, holder))
            {
                return true;
            }
            else
...
struct ReflectionDatabase
{
    ...
    unsigned ResolveIncompleteTypes();

    void ResolveIncompleteType(TypeInstance& t, unsigned& ctr);

    unsigned CountIncompleteTypes() const;
